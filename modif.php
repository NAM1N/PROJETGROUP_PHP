...
if (( $imageReste && $upload && $tab["valid"])||(!$imageReste && $tab["valid"]))
  {
	 ...
	 if ($imageReste )
		{
	     ...
	     $req=$bd->prepare('UPDATE `etudiant`.`T_etudiant` SET `nom_etd` = ?, `pnom_etd` = ?, ``age_etd` = ?,local_etd`=?,`act_etd`=?,`im`=?,groupe=?,date=NOW() WHERE `T_etudiant`.`Id_etd` = ?');
			   // afficher=
		  $req->execute(array($tab["nom"],$tab["pnom"],$tab["age"],$tab["local"],$tab["actu"],$image,$tab["grpe"],$id));
		  $req->closeCursor();
		 ...
		 $bd=New PDO('mysql:host=localhost;dbname=etudiant','root','');
		  ...
		  // mise a jour  des données
	     $req=$bd->prepare('UPDATE `etudiant`.`T_etudiant` SET `nom_etd` = ?, `pnom_etd` = ?, ``age_etd` = ?,local_etd`=?,`act_etd`=?,groupe=?,date=NOW() WHERE `T_etudiant`.`Id_etd` = ?');
		   // afficher
		   $req->execute(array($tab["nom"],$tab["pnom"],$tab["age"],$tab["local"],$tab["actu"],$tab["grpe"],$id));
		  $req->closeCursor();
			}
		 // rediriger vers la page  administration
		      header("location:#.php");
   }
	...
	// recuperer les valeurs
     ...
     $req=$bd->prepare('SELECT * FROM `T_etudiant` WHERE `Id_etd`=?');
			   // afficher
...
// une fonction pour verifier les saisie
...
<body>
<!-- creer le formulaire pour recuperer les données mise a jour par la methode $_POST-->
